...
const NavSearch = () => {
    ...
    const {
        ...
        data,
        isLoading
    } = useQuery('search-input', () => apiService.getData(`product-search?search=${debounceInputValue}`),
        {
            enabled: false
        })
                    ...
                    ? 'block' : 'hidden'} ${data?.results.length > 4 ? 'overflow-y-scroll h-[300px]' : ''}   duration-500 absolute w-full md:max-lg:w-[200%] z-50 top-[115px] md:top-14 md:left-[50%] lg:left-0 right-0 md:max-lg:translate-x-[-50%] bg-white rounded-xl  pb-2`}>
                {
                    data?.count ?
                        ...
                        data?.results?.map(product => (
                            <SearchCardUI key={product?.id} image={product?.image?.image} title_ru={product?.title_ru}
                                          title_uz={product.title_uz} subtitle_ru={product?.brands?.title_ru}
                                          subtitle_uz={product?.brands?.title_uz} href={'/product/' + product?.slug}
