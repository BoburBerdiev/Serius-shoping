        ...
        <div className="flex items-center justify-between max-md:w-full gap-5 lg:gap-10">
          ...
          <div onClick={(e) => handleLanguage(e)} className="lang flex items-center gap-[10px] group cursor-pointer relative z-[60]">
            ...
            <div className="h-full">
              <p className="lang-change max-md:text-xs text-sm">{language}</p>
              <div className={`absolute z-[100] dropdown-list grid grid-rows-[0fr] duration-300 rounded-xl border-white w-full top-6 md:top-[38px] left-[50%] translate-x-[-50%] ${dropdown && 'grid-rows-[1fr] border' }`}>
                <ul className="overflow-hidden rounded-xl bg-darkBlue">
                  <li onClick={ () => handleChangleLang(t('navbar.ru')) } className="duration-300 py-2 px-3 hover:bg-slate-600 cursor-pointer max-md:text-sm flex items-center justify-center gap-1">
                    <div className="h-4 w-4 shrink-0">
                      <img src="/ru-flag.svg" alt="ru" className="w-full h-full object-cover rounded-full" />  
                    </div> 
                    {t('navbar.ru')}
                  </li>
                  <li onClick={ () => handleChangleLang(t('navbar.uz')) } className="duration-300 py-2 px-3 hover:bg-slate-600 cursor-pointer max-md:text-sm flex items-center justify-center gap-1">
                    <div className="h-4 w-4 shrink-0">
